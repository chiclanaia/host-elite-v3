...
VALUES (
        ...
        '1. Pedagogical Objective: Efficiency through events. Operations shouldn''t wait for humans. This teaches users ''Workflow Automation'' and resource optimization. 2. Behavior Matrix: TIER_0: Manual task creation. TIER_1/2: Triggered tasks (e.g., ''Clean after Checkout''). TIER_3: Intelligent Event-Driven Engine with Smart Lock integration and dynamic rescheduling. 3. User Journey & UI: Input: Automation rules (Triggers/Actions). Logic: If event $$Checkout$$ -> Create task $$Clean$$. Output: ''Automation History'' log. Coach: Why ''Early Check-in'' starts with an ''Early Checkout'' detection. 4. Business Logic: RG-01: Sync with OPS_09 (Provider App). RG-02 (Tier 3): Use Smart Lock door-locking events to trigger ''Ready for Cleaning'' status.',
        'GLOBAL'
    ) ON CONFLICT (id) DO
UPDATE
SET parent_feature_id = EXCLUDED.parent_feature_id,
...
VALUES (
        ...
        '1. Pedagogical Objective: 24/7 responsiveness without burnout. This teaches users to scale communication through ''Knowledge Base'' management. 2. Behavior Matrix: TIER_1/2: Automated SMS triggers. TIER_3: Contextual AI Assistant using RAG on property documents with open-source LLM frameworks. 3. User Journey & UI: Input: Chat interface. Logic: Search within $$Welcome Book + Manuals + Previous Chats$$ . Output: Conversational answer. Coach: Why ''Speed of Response'' is the #1 factor in guest satisfaction scores. 4. Business Logic: RG-01 (Tier 3): If AI confidence < 80%, escalate to ''Human Host'' immediately. RG-02: Support for 20+ languages.',
        'GLOBAL'
    ) ON CONFLICT (id) DO
UPDATE
SET parent_feature_id = EXCLUDED.parent_feature_id,
