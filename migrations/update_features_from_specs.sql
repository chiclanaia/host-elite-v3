...
VALUES (
    ...
    'PH_4_OPS',
    'GLOBAL',
    'Smart General Ledger & Expense Sentinel',
    'AI-powered bookkeeping and receipt management engine',
    'Centralized day-to-day financial hub. Captures income, scans expenses, and reconciles bank feeds. TIER 3 uses AI to match booking payouts with bank deposits and generates audit-proof tax data.',
    '1. Pedagogical Objective: Fiscal hygiene. Most audits fail due to missing receipts or bad categorization. This tool teaches ''Real-Time Bookkeeping''—turning a chore into a daily habit. 2. Behavior Matrix: TIER_0: Manual entry form. TIER_1 (Bronze): Interactive Ledger for one unit with CSV import. TIER_2 (Silver): Open Banking Sync (Plaid/GoCardless)—automatically fetches transactions. ''Expense Scanner'' (OCR) for receipts. TIER_3 (Gold): AI Reconciliation Engine. Automatically matches a ''Net Payout'' from Airbnb to the specific bookings and deducts fees, creating a perfect audit trail. 3. User Journey & UI: Input: Bank Feed + Receipt Photo. Logic: Auto-categorize (Capex vs Opex). Output: Live P&L Stream. Coach: ''The Deductible Rule''—explaining what you can and cannot expense. 4. Business Logic: RG-01: TIER_2 for Bank Sync. RG-02: TIER_3 for auto-reconciliation.',
    'TIER_0: Manual entry form. TIER_1 (Bronze): Interactive Ledger for one unit with CSV import. TIER_2 (Silver): Open Banking Sync (Plaid/GoCardless)—automatically fetches transactions. ''Expense Scanner'' (OCR) for receipts. TIER_3 (Gold): AI Reconciliation Engine. Automatically matches a ''Net Payout'' from Airbnb to the specific bookings and deducts fees, creating a perfect audit trail.'
)
ON CONFLICT (id) DO UPDATE SET
    parent_feature_id = EXCLUDED.parent_feature_id,
    dimension_id = EXCLUDED.dimension_id,
