...
@Component({
    ...
    <div class="h-full flex flex-col gap-6 animate-fade-in-up">
      <div class="flex justify-between items-start">
        <div>
          <h1 class="text-3xl font-extrabold text-white tracking-tight">{{ 'CERFA.FrenchAdminAutopilot' | translate }}</h1>
          <p class="text-slate-400 mt-2 max-w-2xl">{{ 'CERFA.CerfaGeneratorMeublDeTourisme' | translate }}</p>
        </div>
       ...
       <div class="flex-1 bg-slate-800 rounded-xl border border-white/10 p-8 flex gap-8">
            ...
            <div class="w-64 space-y-4">
                <h3 class="text-white font-bold text-sm uppercase opacity-50 mb-4">{{ 'CERFA.AvailableForms' | translate }}</h3>
                <button class="w-full text-left p-3 rounded bg-blue-600 text-white font-bold text-xs flex items-center justify-between shadow-lg shadow-blue-500/20 ring-1 ring-blue-400">
                    <span>{{ 'CERFA.Cerfa1400404' | translate }}</span>
                ...
                <button class="w-full text-left p-3 rounded bg-white/5 hover:bg-white/10 text-slate-400 text-xs flex items-center justify-between transition-colors">
                     <span>{{ 'CERFA.Cerfa1181903Tax' | translate }}</span>
                </button>
                <button class="w-full text-left p-3 rounded bg-white/5 hover:bg-white/10 text-slate-400 text-xs flex items-center justify-between transition-colors">
                     <span>{{ 'CERFA.TouristTaxReceipt' | translate }}</span>
                </button>
                
                @if (isTier2()) {
                    <div class="mt-8 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">
                        <h4 class="text-indigo-300 font-bold text-xs mb-2">{{ 'CERFA.SmartSelector' | translate }}</h4>
                        <p class="text-[10px] text-indigo-400">{{ 'CERFA.BasedOnYourParisLocation' | translate }}</p>
                    </div>
                }
            </div>
            ...
            <div class="flex-1 bg-white text-slate-900 p-8 rounded shadow-2xl relative min-h-[500px] transform transition-all duration-500"
                 ...
                 <div class="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-end backdrop-blur-sm">
                     <div>
                         <div class="text-xs uppercase font-bold tracking-widest text-slate-500">{{ 'CERFA.CerfaN1400404' | translate }}</div>
                         <h2 class="text-2xl font-serif font-bold mt-1">{{ 'CERFA.DclarationDeMeublDeTourisme' | translate }}</h2>
                     </div>
                 ...
                 <div class="grid grid-cols-2 gap-8 relative">
                     ...
                     <div class="space-y-4 z-10">
                         <div>
                             <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">{{ 'CERFA.IdentitDuDclarant' | translate }}</label>
                          ...
                          <div>
                             <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">{{ 'CERFA.AdresseDuMeubl' | translate }}</label>
                     ...
                     <div class="space-y-4 z-10">
                         <div>
                             <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">{{ 'CERFA.CapacitDaccueil' | translate }}</label>
                         ...
                         <div>
                             <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">{{ 'CERFA.Classement' | translate }}</label>
                 ...
                 <div class="mt-8 pt-8 border-t border-slate-200">
                     <div class="bg-slate-100 p-4 rounded text-xs text-slate-600 mb-4">
                         <p class="mb-2 font-bold">{{ 'CERFA.Disclaimer' | translate }}</p>
                 ...
                 <div class="absolute bottom-8 right-8 flex gap-4 z-20">
                      @if (isTier3()) {
                          <button class="bg-indigo-600 text-white px-6 py-3 rounded shadow-lg font-bold text-sm flex items-center gap-2 hover:bg-indigo-700 transition-all hover:scale-105" data-debug-id="cerfa-submit-btn">
                              <span class="material-icons text-sm">send</span>{{ 'CERFA.AutosubmitToCityHall' | translate }}</button>
                      } @else {
                           @if (isTier2()) {
                                <button class="bg-slate-800 text-white px-6 py-3 rounded shadow-lg font-bold text-sm flex items-center gap-2 hover:bg-slate-700" data-debug-id="cerfa-download-btn">
                                  <span class="material-icons text-sm">download</span>{{ 'CERFA.DownloadPrefilledPdf' | translate }}</button>
                           } @else {
                                <button class="bg-slate-200 text-slate-400 px-6 py-3 rounded font-bold text-sm flex items-center gap-2 cursor-not-allowed" disabled>
                                    <span class="material-icons text-sm">lock</span>{{ 'CERFA.UpgradeToFill' | translate }}</button>
                           }
                      }
                 </div>
            </div>
        ...
        <div class="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 mt-6">
           <div class="flex items-start gap-3">
              ...
              <div>
                  <h4 class="font-bold text-indigo-300 text-sm">{{ 'CERFA.TaxHackClassifiedTourism' | translate }}</h4>
...
export class CerfaGeneratorComponent {
    ...
    feature = computed(() => ({
        id: 'LEG_03',
        name: this.translate.instant('CERFGENE.Title'),
        description: this.translate.instant('CERFGENE.Description'),
    } as any));
