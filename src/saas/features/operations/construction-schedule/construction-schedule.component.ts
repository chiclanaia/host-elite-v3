       ...
       <div class="flex-1 bg-slate-800 rounded-xl border border-white/10 p-6 flex flex-col">
            ...
            <div class="flex-1 space-y-2 overflow-y-auto">
                 ...
                 <div class="flex items-center gap-4">
                     <div class="w-32 text-xs text-white font-bold truncate">{{ 'CONSTRUCTI.Demolition' | translate }}</div>
                     <div class="flex-1 bg-black/30 h-8 rounded relative">
                         <div class="absolute left-0 w-[20%] h-full bg-emerald-500 rounded flex items-center px-2 text-[10px] text-black font-bold">{{ 'CONSTRUCTI.Done' | translate }}</div>
                     </div>
                 </div>

                  <!-- Task 2 -->
                 <div class="flex items-center gap-4">
                     <div class="w-32 text-xs text-white font-bold truncate">{{ 'CONSTRUCTI.Plumbing' | translate }}</div>
                     <div class="flex-1 bg-black/30 h-8 rounded relative">
                         <div class="absolute left-[20%] w-[15%] h-full bg-amber-500 rounded flex items-center px-2 text-[10px] text-black font-bold">{{ 'CONSTRUCTI.InProgress' | translate }}</div>
                     </div>
                 </div>
                 
                 <!-- Task 3 -->
                  <div class="flex items-center gap-4">
                     <div class="w-32 text-xs text-slate-500 truncate">{{ 'CONSTRUCTI.Painting' | translate }}</div>
                     <div class="flex-1 bg-black/30 h-8 rounded relative">
                         <div class="absolute left-[35%] w-[30%] h-full bg-slate-600 rounded border border-white/10 flex items-center px-2 text-[10px] text-slate-300">{{ 'CONSTRUCTI.Scheduled' | translate }}</div>
