        ...
        <div class="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
            <div>
                <h2 class="text-2xl font-bold text-white flex items-center gap-3">
                    <span class="text-3xl">🤖</span> Générateur de F.A.Q.
                </h2>
                <p class="text-slate-400 text-sm mt-1">Anticipez les besoins de vos voyageurs avec des questions/réponses générées par l'IA.</p>
            </div>
        ...
        <div class="flex-1 p-6 overflow-y-auto space-y-6">
            ...
            @if (isLoadingData()) {
                <div class="flex items-center space-x-2 text-blue-400 animate-pulse">
                    ...
                    <span>Analyse de la propriété et du quartier...</span>
                </div>
            } @else {
                <div class="bg-blue-500/10 border border-blue-500/20 text-blue-200 px-4 py-2 rounded-lg text-xs flex items-center justify-between">
                    <div class="flex flex-col gap-1">
                         <span class="flex items-center">
                            ...
                            Contexte chargé : {{ contextSummary() }}
                        </span>
                        <span class="text-[10px] text-blue-300 pl-6 opacity-75 flex items-center gap-2">
                            Adresse utilisée : 
                            <input [(ngModel)]="propertyAddress" class="bg-black/20 border border-white/10 rounded px-2 py-0.5 text-white text-xs w-64 focus:outline-none focus:border-blue-400 dashed-border">
                        </span>
                    </div>
                    <button class="text-blue-400 hover:text-white underline ml-4 whitespace-nowrap" (click)="loadPropertyData()">Réinitialiser</button>
                </div>
            }
            ...
            @if (generatedFaqs().length === 0) {
                <div class="text-center py-12">
                    <div class="bg-white/5 inline-flex p-4 rounded-full mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-blue-400">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-white mb-2">Aucune F.A.Q. générée</h3>
                    <p class="text-slate-400 max-w-sm mx-auto mb-6">Laissez l'IA analyser votre propriété et les forums de voyageurs pour créer les 5 questions les plus pertinentes.</p>
                ...
                <div class="flex justify-between items-center mb-2">
                    <h3 class="text-slate-300 font-bold uppercase tracking-wide text-sm">Questions Générées ({{ generatedFaqs().length }})</h3>
                    <button (click)="generateFaqs()" class="text-xs text-blue-400 hover:text-white flex items-center gap-1 transition-colors" [disabled]="isGenerating()">
                ...
                <div class="space-y-4">
                    @for (item of generatedFaqs(); track $index) {
                        <div class="bg-black/30 border border-white/10 rounded-xl p-4 animate-fade-in-up" [style.animation-delay]="$index * 100 + 'ms'">
                            <div class="mb-3">
                                <label class="block text-xs font-bold text-blue-400 mb-1">QUESTION</label>
                                <input [(ngModel)]="item.question" class="w-full bg-transparent border-b border-white/10 focus:border-blue-400 text-white font-semibold py-1 focus:outline-none transition-colors">
                            </div>
                            <div class="mb-3">
                                <label class="block text-xs font-bold text-emerald-400 mb-1">RÉPONSE SUGGÉRÉE</label>
                                <textarea [(ngModel)]="item.answer" rows="3" class="w-full bg-white/5 rounded-lg p-2 text-sm text-slate-200 focus:bg-white/10 focus:outline-none transition-colors resize-none"></textarea>
                            </div>
                            <div class="flex justify-end gap-3 pt-2 border-t border-white/5">
                                <button (click)="deleteItem($index)" class="text-xs text-red-400 hover:text-red-300">Supprimer</button>
                                <button (click)="copyItem(item)" class="text-xs text-slate-400 hover:text-white flex items-center gap-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
    ...
    async loadPropertyData() {
        ...
        try {
            ...
            `;
                ...
                this.propertyContext = "";
                this.contextSummary.set("Erreur: Propriété introuvable.");
            }
        } catch (e) {
            console.error("Error loading context", e);
            this.contextSummary.set("Erreur de chargement.");
        } finally {
