...
export class SupabaseService {
  ...
  async uploadAvatar(userId: string, file: File): Promise<{ publicUrl: string | null; error: any }> {
    if (!this._isConfigured) return { publicUrl: null, error: { message: "Supabase not configured." } };

    const fileExt = file.name.split('.').pop();
    const fileName = `${userId}/avatar_${Math.random().toString(36).substring(2)}.${fileExt}`;
